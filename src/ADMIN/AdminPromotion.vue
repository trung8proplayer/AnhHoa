<template>
  ...
  <tr v-for="item in promotions" :key="item._id">
    ...
    <td><img :src="item.promotion_image" alt="" style="height:100px"></td>
    <td>{{item.content}}</td>
    <td>
        <div style="display:flex">
          <button @click.prevent="deletePromotion(item._id)" class="btn-delete">Xóa </button>
          <button v-b-modal.modal-prevent @click.prevent="editPromotion(item._id)" class="btn-edit">Sửa </button>
        </div>
    </td>
  </tr>
</table>
...
export default {
  ...
  methods: {
      ...
      async updatePromotion(){
        axios.put(`promotions/${this.oldPromotion._id}`,this.oldPromotion).then(()=>{
          for(let i=0; i < this.promotions.length; i++){
